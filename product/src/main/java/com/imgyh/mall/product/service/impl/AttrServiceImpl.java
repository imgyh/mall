...
public class AttrServiceImpl extends ServiceImpl<AttrDao, AttrEntity> implements AttrService {
    ...
    @Override
    public PageUtils queryListPage(Map<String, Object> params, Long catelogId, String attrType) {

        // attrType 销售属性还是基本属性
        // 销售属性不会查询 attrAttrGroup 表
        Integer attrCode = "base".equalsIgnoreCase(attrType) ?
                ProductConstant.AttrType.BASE_TYPE.getCode() : ProductConstant.AttrType.SALE_TYPE.getCode();
        // 查找Attr表中的信息
        String key = (String) params.get("key");
        QueryWrapper<AttrEntity> wrapper = new QueryWrapper<AttrEntity>().eq("attr_type", attrCode);
        ...
        List<AttrResponseVo> list = records.stream().map(
                (attrEntity) -> {
                    ...
                    if (attrCode.equals(ProductConstant.AttrType.BASE_TYPE.getCode())) {
